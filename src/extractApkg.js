...
if (args.length < 1) {
  console.error(
    "Uso: node extractApkg.js '<caminho_do_arquivo.apkg>' OU npm run extract '<caminho_do_arquivo.apkg>'"
  );
  process.exit(1);
}
...
const outputDir = path.resolve(
  __dirname,
  `../decks/${fileNameWithoutExtension}`
);

extractAPKG(filePath, outputDir);
